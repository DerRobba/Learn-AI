...
def ask():
    ...
    try:
        ...
        # Check if AI wants to add vocabulary
        if answer.startswith("ADDVOCAB:"):
            vocab_string = answer.replace("ADDVOCAB:", "").strip()
            new_vocab = []
            pairs = vocab_string.split(',')
            for p in pairs:
                parts = p.split(':')
                if len(parts) == 2:
                    ...
                    if german and english:
                        new_vocab.append({'german': german, 'english': english})
            
            if new_vocab:
                if 'vocabulary' not in session:
                    session['vocabulary'] = []
                session['vocabulary'].extend(new_vocab)
                session.modified = True
                # Overwrite answer with a confirmation message
                answer = f"Ich habe {len(new_vocab)} Vokabeln zu deinem Vokabeltrainer hinzugefügt."
...
def add_vocabulary():
    ...
    if request.is_json:
        ...
        if isinstance(new_vocab, list):
            session['vocabulary'].extend(new_vocab)
            session.modified = True
            return jsonify({'success': True, 'message': f'{len(new_vocab)} Vokabeln hinzugefügt.'})
        else:
            return jsonify({'success': False, 'message': 'Ungültiges Datenformat.'}), 400
    else:
        return jsonify({'success': False, 'message': 'Nur JSON-Daten werden unterstützt.'}), 400

@app.route('/delete-vocabulary', methods=['POST'])
def delete_vocabulary():
    if 'user_id' not in session:
        return jsonify({'error': 'Not logged in'}), 401

    if 'vocabulary' not in session:
        return jsonify({'success': False, 'message': 'Keine Vokabeln zum Löschen vorhanden.'}), 400

    data = request.get_json()
    word_to_delete = data.get('word')

    if not word_to_delete or not isinstance(word_to_delete, dict):
        return jsonify({'success': False, 'message': 'Ungültiges Vokabelformat zum Löschen.'}), 400

    # Remove the word from the session vocabulary list
    # We need to compare the content of the dicts, not just the dicts themselves
    initial_len = len(session['vocabulary'])
    session['vocabulary'] = [w for w in session['vocabulary'] if not (
        w.get('german') == word_to_delete.get('german') and 
        w.get('english') == word_to_delete.get('english')
    )]
    
    if len(session['vocabulary']) < initial_len:
        session.modified = True
        return jsonify({'success': True, 'message': 'Vokabel erfolgreich gelöscht.'})
    else:
        return jsonify({'success': False, 'message': 'Vokabel nicht gefunden.'}), 404

@app.route('/get-vocabulary-lists')
def get_vocabulary_lists():
    if 'user_id' not in session:
        return jsonify({'error': 'Not logged in'}), 401
    
    lists = session.get('vocabulary_lists', {})
    return jsonify({'lists': lists})

@app.route('/save-vocabulary-list', methods=['POST'])
def save_vocabulary_list():
    if 'user_id' not in session:
        return jsonify({'error': 'Not logged in'}), 401

    data = request.get_json()
    list_name = data.get('name')
    vocab_to_save = data.get('vocabulary', [])

    if not list_name:
        return jsonify({'success': False, 'message': 'Listenname ist erforderlich.'}), 400
    if not isinstance(vocab_to_save, list):
        return jsonify({'success': False, 'message': 'Ungültiges Vokabelformat.'}), 400

    if 'vocabulary_lists' not in session:
        session['vocabulary_lists'] = {}
    
    session['vocabulary_lists'][list_name] = vocab_to_save
    session.modified = True
    return jsonify({'success': True, 'message': f'Liste \'{list_name}\' erfolgreich gespeichert.'})

@app.route('/load-vocabulary-list', methods=['POST'])
def load_vocabulary_list():
    if 'user_id' not in session:
        return jsonify({'error': 'Not logged in'}), 401
    ...
    if list_name in lists:
        session['vocabulary'] = lists[list_name]
        session.modified = True
        return jsonify({'success': True, 'message': f'Liste \'{list_name}\' erfolgreich geladen.'})
    else:
        return jsonify({'success': False, 'message': 'Liste nicht gefunden.'}), 404

@app.route('/delete-vocabulary-list', methods=['POST'])
def delete_vocabulary_list():
    ...
    if 'vocabulary_lists' in session and list_name in session['vocabulary_lists']:
        del session['vocabulary_lists'][list_name]
        session.modified = True
        return jsonify({'success': True, 'message': f'Liste \'{list_name}\' erfolgreich gelöscht.'})
    else:
        return jsonify({'success': False, 'message': 'Liste nicht gefunden.'}), 404
