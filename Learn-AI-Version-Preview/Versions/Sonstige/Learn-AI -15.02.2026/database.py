...
def init_database():
    ...
    # Add subject column to chat_history table if it doesn't exist
    try:
        cursor.execute("ALTER TABLE chat_history ADD COLUMN subject TEXT")
    except sqlite3.OperationalError as e:
        if 'duplicate column name' not in str(e):
            pass # Column already exists
...
def save_chat_message(user_id, session_id, message_type, content, image_data=None, worksheet_filename=None, subject=None):
    """Save a chat message to the database"""
    conn = sqlite3.connect(DATABASE_PATH)
    cursor = conn.cursor()

    cursor.execute('''
        INSERT INTO chat_history (user_id, session_id, message_type, content, image_data, worksheet_filename, subject)
        VALUES (?, ?, ?, ?, ?, ?, ?)
    ''', (user_id, session_id, message_type, content, image_data, worksheet_filename, subject))

    conn.commit()
    conn.close()

def get_chat_history(user_id, session_id, subject=None):
    """Get chat history for a specific user and session, optionally filtered by subject"""
    conn = sqlite3.connect(DATABASE_PATH)
    cursor = conn.cursor()
...
def get_user_chat_sessions(user_id):
    ...
    cursor.execute('''
        SELECT 
            ...
            COALESCE(session_name, (SELECT content FROM chat_history WHERE session_id = T1.session_id ORDER BY created_at ASC LIMIT 1)) as session_name,
            (SELECT subject FROM chat_history WHERE session_id = T1.session_id AND subject IS NOT NULL ORDER BY created_at ASC LIMIT 1) as subject
        FROM chat_history AS T1
        WHERE user_id = ?
        GROUP BY session_id
    ...
    return [{'session_id': s[0], 'first_message': s[1], 'last_message': s[2], 'session_name': s[3], 'subject': s[4]} for s in sessions]
...
def get_chat_session_subject(user_id, session_id):
    """Get the subject of a specific chat session"""
    conn = sqlite3.connect(DATABASE_PATH)
    cursor = conn.cursor()
    cursor.execute('''
        SELECT subject FROM chat_history
        WHERE user_id = ? AND session_id = ? AND subject IS NOT NULL
...
def get_unique_chat_subjects(user_id):
    """Get all unique subjects for a user's chat sessions"""
    conn = sqlite3.connect(DATABASE_PATH)
    cursor = conn.cursor()
    cursor.execute('''
        SELECT DISTINCT subject FROM chat_history
...
def get_chat_sessions_by_subject(user_id, subject):
    """Get all chat sessions for a user, filtered by subject"""
    conn = sqlite3.connect(DATABASE_PATH)
    cursor = conn.cursor()
