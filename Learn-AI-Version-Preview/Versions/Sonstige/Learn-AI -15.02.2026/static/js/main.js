document.addEventListener('DOMContentLoaded', function() {
    ...
    const currentSessionId = "{{ current_session_id }}"; // Get current session ID from Flask

    function renderChatSessions(sessions) {
        chatSessionsContainer.innerHTML = '';
        if (sessions.length === 0) {
            chatSessionsContainer.innerHTML = '<p class="text-center text-gray-500 text-sm mt-4">Keine Chats für dieses Thema gefunden.</p>';
            return;
        }
        ...
        sessions.forEach(session => {
            ...
            sessionElement.innerHTML = `
                <div class="flex flex-col space-y-1 pointer-events-none">
                    ...
                    <span class="session-date text-xs text-gray-500">${session.last_message.substring(0, 16)}</span>
                    ${session.subject ? `<span class="session-subject text-xs text-purple-600">Thema: ${session.subject}</span>` : ''}
                </div>
            `;
    ...
    if (subjectFilter) {
        subjectFilter.addEventListener('change', (event) => {
            ...
            if (selectedSubject === '') {
                // Fetch all chat sessions if "Alle Chats" is selected
                url = '/api/all-chat-sessions';
            } else {
                // Fetch sessions filtered by subject
                url = `/api/chat-sessions-by-subject?subject=${encodeURIComponent(selectedSubject)}`;
            }

            fetch(url)
                .then(response => response.json())
                .then(data => {
                    renderChatSessions(data.chat_sessions);
                })
                .catch(error => {
                    console.error('Error fetching filtered chat sessions:', error);
                });
        });
    }
});