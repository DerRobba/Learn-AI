...
subject_system_prompt = "Sie sind ein hilfreicher Assistent, der Chat-Themen identifiziert. Wenn Sie eine Benutzerfrage erhalten, antworten Sie mit einem sehr kurzen, prägnanten Thema (maximal 3-5 Wörter) für die Konversation. Antworten Sie NUR mit dem Thema und nichts anderem. Zum Beispiel: 'Mathematik-Aufgabe', 'Geschichte des Mittelalters', 'Biologie-Fragen'. Wenn die Frage nicht thematisch ist, antworten Sie mit 'Allgemeiner Chat'."

def get_ai_generated_subject(initial_prompt):
    try:
        ...
        print(f"Error generating subject with AI: {e}")
        return "Allgemeiner Chat" # Default subject in case of error
...
def ask():
    ...
    # Determine the subject for the current chat session
    current_chat_subject = get_chat_session_subject(user_id, chat_session_id)
    if not current_chat_subject:
        current_chat_subject = get_ai_generated_subject(question)
        # Store the subject in the session for subsequent messages
        session['current_chat_subject'] = current_chat_subject
    else:
        session['current_chat_subject'] = current_chat_subject

    def generate():
        try:
            # Get existing chat history for context, filtered by subject
            chat_history = get_chat_history(user_id, chat_session_id, session.get('current_chat_subject'))
            ...
            # Save user question to database, including the subject
            image_data = None
            if cached_image:
                image_data = f"data:{cached_image['mime_type']};base64,{cached_image['base64']}"

            save_chat_message(user_id, chat_session_id, 'user', question, image_data, subject=session.get('current_chat_subject'))
            ...
            if md_content:
                ...
                save_chat_message(user_id, chat_session_id, 'assistant', download_answer, worksheet_filename=pdf_basename, subject=session.get('current_chat_subject'))
                ...
                save_chat_message(user_id, chat_session_id, 'assistant', full_answer, subject=session.get('current_chat_subject'))
...
@app.route('/api/chat-sessions-by-subject', methods=['GET'])
def get_chat_sessions_by_subject_route():
    if 'user_id' not in session:
        return jsonify({'error': 'Nicht angemeldet'}), 401
    ...
    chat_sessions = get_chat_sessions_by_subject(user_id, subject)
    return jsonify({'chat_sessions': chat_sessions})

@app.route('/api/all-chat-sessions', methods=['GET'])
def get_all_chat_sessions_route():
    if 'user_id' not in session:
        return jsonify({'error': 'Nicht angemeldet'}), 401
...
@app.route('/worksheet-preview/<filename>')
def worksheet_preview(filename):
    try:
        # Sanitize filename to prevent directory traversal
        filename = os.path.basename(filename)
        filepath = os.path.join('sheets', filename)
        if not os.path.exists(filepath) or not filepath.endswith('.md'):
            return abort(404)
        with open(filepath, 'r', encoding='utf-8') as f:
            content = f.read()
        return render_template('worksheet_preview.html', markdown_content=content)
    except Exception as e:
        print(f"Error serving worksheet preview: {e}")
        return abort(500)
...
def index():
    ...
    if 'chat_session_id' not in session:
        ...
        session['current_chat_subject'] = "Allgemeiner Chat" # Default subject for new sessions

    # Get user's chat sessions
    user_sessions = get_user_chat_sessions(session['user_id'])

    # Get current chat subject (if available in session)
    current_chat_subject = session.get('current_chat_subject', "Kein Thema ausgewählt")
    ...
    unique_subjects = get_unique_chat_subjects(session['user_id'])
    ...
    return render_template('index.html',
                         ...
                         current_chat_subject=current_chat_subject,
